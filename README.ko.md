...
# Brain-AI Memory — 다음 세션까지 이어지는 프로젝트 메모리

**현재 사실, 정확한 상태값, 결정, 다음 작업을 세션 사이에 이어 씁니다.
프로젝트 기억은 섞지 않고 기존 `MEMORY.md`는 그대로 둡니다.**

Brain-AI Memory는 Codex, Claude Code와 다른 MCP 호스트를 위한 로컬 메모리
계층입니다. 기존 Markdown 메모리를 검토해 승인한 기록만 저장하고, 프로젝트별
MCP 설정을 만들어 주며, 다음 세션이 이어받을 인계 내용을 남깁니다. 바뀐 사실도
조용히 지우지 않고 출처 이력과 함께 보존합니다.

**Local-first · API key 불필요 · hosted service 불필요 · 외부 database server
불필요 · Codex / Claude Code / MCP · Python 3.10+ · MIT**
...
<p align="center">
  세션 기록 → 프로젝트와 시점에 맞는 기억 → 다음 세션을 위한 handoff.<br>
  아래쪽은 선택 기능인 행동 검사입니다.
</p>

> 검색은 관련 문장을 찾습니다. 세션을 넘는 메모리에는 프로젝트 범위, 현재
> version 선택, 정확한 상태값, 출처 이력, 다음 세션을 위한 handoff가 더 필요합니다.
...
**공개 소프트웨어 검증:** 64개 테스트, Python 3.10–3.12 호환성, clean wheel의
audit부터 실제 MCP 재시작·resume까지 이어지는 통합 흐름, component contract
20/20을 확인했습니다. 이는 설치·통합 근거이며 Codex·Claude Code의 tool 선택이나
더 나은 LLM 답변을 입증한 결과는 아닙니다.
...
내려받지 않습니다. Vault·Smart Connections 연동은 선택 사항입니다. 호스트가
도구를 직접 호출해야 하며, Brain-AI Memory가 대화를 몰래 읽거나 문맥을 자동으로
주입하지 않습니다.

## 기존 MEMORY.md 가져오기

위 tour를 실행하지 않았다면 저장소를 내려받고 MCP 기능까지 함께 설치합니다.
...
메모리 파일을 사용하는 프로젝트에서 이후 전체 작업이 쓸 프로젝트 경로와
로컬 저장소를 고정하고, 기록이 속할 대상을 하나 지정합니다.
...
| 실행 규칙 | `--rule ITEM=REGEX`와 `--rule-effect warn\|block` |
...
연결 명령도 먼저 diff를 보여 줍니다. 기본 범위는 현재 프로젝트이며, 지정한
entity가 MCP server의 프로젝트 기본 메모리 범위로 설정됩니다. 이 entity는
같은 `BRAIN_AI_HOME`에 먼저 있어야 합니다. Adoption `apply`가 성공하면
자동으로 생기며, adoption을 건너뛰었다면
`brain-ai entity add --name my-project --type project`를 한 번 실행합니다.

```bash
# Codex: 미리보기 후 .codex/config.toml에 적용
...
# Claude Code: 미리보기 후 .mcp.json에 적용
...
이 부분으로 바로 왔다면 먼저 `cd /path/to/your/project`,
`export PROJECT_ROOT="$PWD"`,
`export BRAIN_AI_HOME="$PROJECT_ROOT/.brain-ai"`를 실행해 entity 생성, 연결, MCP server가
같은 저장소를 쓰게 합니다.

이 프로젝트 밖에서도 하나의 store와 entity를 기본으로 쓸 때만 `--scope user`를
사용하세요. `brain-ai disconnect ...`도 먼저 diff를 보여 주고 `--apply`가 있어야
설정을 바꿉니다.
...
MCP entry는 이 command의 관리 대상이 아니므로 직접 수정하거나 제거해야 합니다.
...
호스트가 생성된 설정을 불러오거나 승인하면 MCP server가 `brain_context`,
`brain_remember`, `brain_checkpoint`, `brain_resume`을 제공하고 프로젝트 entity를
기본 범위로 사용합니다. MCP tool이 보인다고 해서 대화 기록이 자동으로 수집되거나
tool 호출이 보장되는 것은 아닙니다. 무엇을 불러오고, 문맥에 넣고, 저장할지는
호스트가 정합니다.

호스트가 일관된 메모리 흐름을 따르게 하려면 프로젝트의 `AGENTS.md`나
`CLAUDE.md`에 다음과 같은 지시문을 추가하세요.
...
| 수명 주기와 세션 인계 | 통합, 재통합, 활성·비활성 결정, rollback 근거, entity별 handoff·resume |
| 호스트 연결 | 기본 entity 범위가 있는 Codex·Claude Code 프로젝트 MCP 설정을 미리 보고 적용 |
...
현재 test suite는 adoption workflow 통합 사례 36개, runtime 사례 22개, ablation
사례 5개, packaged MCP 재시작·resume 사례 1개(총 64개)를 확인합니다. 아직 RAG나
...
Brain-AI Memory는 adoption과 메모리 기능을 제공하고, 전체 흐름은 호스트가
실행합니다.

1. 기존 `MEMORY.md`를 audit·review하거나, 원본 대화는 호스트에 두고 다음
   세션까지 가져갈 기록만 고릅니다.
2. 승인한 import를 apply하거나 기억 유형, 대상, 출처, 필요한 상태값과 함께
...
6. 승격할 내용을 확인하고, 오래된 지식을 교체하고, 보관·분할·압축·이동·논리
   삭제 결정을 기록합니다.
7. 다음 세션이 읽을 entity별 handoff를 남기고, 작업을 계속할 때 같은 범위를
   resume합니다.

현재 패키지는 Markdown audit·review·apply workflow, Codex·Claude Code의 관리형
프로젝트 MCP 설정, 이 호출들, 감사 기록을 제공합니다. 제공자 대화 기록을
자동으로 가져오지는 않습니다. 일정 관리, 토큰 예산, 파일 보존, handoff 확인은
호스트가 맡습니다. 기록 형식과 세션 인계 방식은
[메모리 수명 주기](docs/02-memory-lifecycle.ko.md)를 참고하세요.
...
현재 공개 버전은 로컬 adoption workflow이자 메모리 커널입니다. 기존
`MEMORY.md`를 audit·import하고, Codex나 Claude Code에 프로젝트 범위 MCP를
설정하며, 범위가 정해진 handoff를 기록하고 resume할 수 있습니다. 전체 메모리
흐름을 자동으로 운영하는 서비스는 아직 아닙니다. 다음 순서로 기능을 추가할
계획입니다.
...
4. handoff 소비·확인 상태와 수명 주기 backlog·상태 지표 추가
5. 압축, 분할, 보관, 검증된 삭제 어댑터를 제공하고 전체 호스트 흐름을 끝까지
   테스트

여러 프로세스의 동시 쓰기, 인증, 도메인 온톨로지 추론, 엔터티 병합·버전 관리는
이후 과제입니다. 현재 범위는 **설치 가능하고 review를 우선하는 로컬 메모리
workflow와 커널**입니다. 완전 자동 메모리 플랫폼은 아직 아닙니다.

...
**프로젝트 상태:** 2026년 4월 20일부터 실제로 매일 사용해 온 시스템에서
clean-room 방식으로 추출한 공개판입니다. 이 저장소는 아키텍처, 재사용 가능한
템플릿, 실행 가능한 예제를 제공하지만 비공개 운영 백엔드나 원시 데이터는
공개하지 않습니다. 검증 현황은 아래에 구분해 제시합니다.
...
패키지 설치, API key, 모델 호출이 필요하지 않습니다.
...
첫 번째 예제는 기억된 안전 규칙을 결정론적 판단으로 바꿉니다.
...
두 번째 예제는 계속 쌓이는 메모리를 명시적인 생명주기 결정으로 바꿉니다.
...
이 작은 예제들은 전체 비공개 운영 백엔드가 아니라 핵심 계약을 보여줍니다.
여러분의 에이전트 스택에 적용하기 전에 이 구분이 실제로 유용한지 판단할 수
있습니다.
...
| Brain-AI Memory | 어느 subsystem이 실패했고 어떤 mechanism과 lifecycle operation이 맞는가? | 설계를 실행하려면 여전히 여러분의 RAG, hook, store, harness runtime이 필요함 |
...
실제 시스템은 구현되어 있지만, 이 clean-room 공개판은 설치 가능한 비공개
백엔드 대신 pattern과 template을 제공합니다. 한 가지 결과부터 시작하세요.
...
| [benchmarks/](benchmarks/) | 평가 protocol, retrieval pilot, release gate |

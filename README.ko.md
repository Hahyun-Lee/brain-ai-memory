...
PyPI에서 패키지를 설치한 뒤, 실제 메모리 파일을 건드리기 전에 임시 디렉터리에서 tour를
실행해 보세요.
...
Source checkout을 사용할 때는 이 저장소를 clone한 뒤 그 안에서 `python -m pip
install .`을 실행하면 됩니다.
...
**검증 범위:** 전체 135개 테스트는 Python 3.12에서 실행하며, 핵심
...
위 tour를 실행하지 않았다면 기본 패키지를 설치합니다.
...
Source checkout을 사용할 때는 위 설치 command 대신 저장소 안에서 `python -m pip
install ".[mcp]"`를 실행하세요.

Claude Code에서는 `codex`를 `claude-code`로 바꾸면 됩니다. 기존 `MEMORY.md`를
가져왔다면 setup은 기존 project entity를 재사용하며 memory를 임의로 import하거나
승인하지 않습니다. 첫 command는 아무것도 쓰지 않는 preview입니다. `--apply`는
필요할 때만 빈 project entity를 만들고, 검토한 config diff를 적용한 뒤 `doctor`까지
실행합니다. Codex에서는 `/hooks`로 이 프로젝트에 추가된 hook을 확인하고 신뢰한 뒤
새 세션을 시작하세요. `doctor`는 설치 직후 `configured`, 실제 start → prompt →
stop이 한 번 관찰된 뒤 `active`로 표시합니다.
...
승인해 가져온 `MEMORY.md`가 바뀌면 세션 시작 때와 지원하는 파일 수정 뒤에 source
fragment를 다시 확인합니다.
현재 파일에서 사라진 fragment로부터 가져온 사실·상태·규칙은 이력에는 남기되 자동
recall에서는 제외합니다. Import rule의 원본이 바뀌면 승인한 guard를 조용히 없애지
않고 지원하는 action을 review 전까지 fail-closed로 보류합니다. 바뀐 파일은 기존 audit 형식으로 준비되므로 새
항목이나 교체 항목만 review·reconsolidation할 수 있습니다. 그대로 남은 fragment는
계속 사용합니다. 오래된 기억을 현재 기억처럼 쓰지 않으면서도 프로그램이 문장의
진실을 임의로 판단하지 않는 방식입니다.

필요할 때 agent가 직접 memory tool을 호출하게 하려면 tools mode를 명시하세요.
...
결정합니다. 고급 설정에는 하위 수준 `connect` command를 그대로 사용할 수
있으며, `--scope user`는 의도적인 tools-only 연결에서만 지원합니다. `brain-ai
disconnect ...`도 먼저 diff를 보여 주고 `--apply`가 있어야 설정을 바꿉니다.
...
현재 test suite는 adoption workflow 43개, runtime 27개, ablation 5개, packaged
재시작·resume 1개, host integration 23개, automatic loop 32개, storage
내구성·동시성 4개(총 135개)를 확인합니다. 아직 RAG나

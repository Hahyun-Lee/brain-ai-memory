...
先从仓库安装，再在临时目录中运行 tour，不会触碰真实的记忆文件：
...
**已验证内容：**完整的 123 个测试在 Python 3.12 上运行；核心 runtime 和
...
export BRAIN_AI_HOME="$PROJECT_ROOT/.brain-ai"
brain-ai init
brain-ai entity add --name my-project --type project  # 已存在时跳过

# Codex：预览、应用，然后在此项目中打开新的 Codex 会话
...
`MEMORY.md`，导入过程会创建项目 entity。Codex 会要求你信任准确的项目 hooks；
请先用 `/hooks` 检查，再开始新的会话。安装后 `doctor` 会显示 `configured`；
只有观察到一次真实的 start → prompt → stop 循环后才会显示 `active`。
...
如果只需要按需调用 memory tools，可以省略 `--mode loop`：
...
`--scope user` 只适用于这种有意配置的 tools-only 模式。`disconnect` 同样先显示
预览，只有加上 `--apply` 才会修改 host 配置。
...
当前测试包括 40 个 adoption-workflow、27 个 runtime、5 个 ablation、1 个 packaged
restart/resume、17 个 host-integration、29 个 automatic-loop，以及 4 个 storage
durability/concurrency 测试，共 123 个。它们还没有证明真实 LLM agent 的端到端答案

...
absolute Brain-AI home, **잠긴 project entity**를 포함합니다. 이 project connection의
tool call은 entity를 override할 수 없으므로 prompt나 tool call이 다른 project의
memory로 조용히 넘어갈 수 없습니다. 이 machine-local path는
...
User-level tools-only connection이 의도된 경우에만 `--scope user`를 사용하세요.
Project scope와 달리 생성된 user connection은 `--entity`를 overridable default로
사용합니다. User-level server 하나가 여러 entity를 의도적으로 제공할 수 있기
때문입니다. Autonomous loop mode는 계속 project-only입니다. 수동 client 설정은
보통 다음과 같습니다.
...
v0.6 이전에 만든 managed project connection은 override 가능한 `--entity` 형식을
사용했습니다. `brain-ai doctor --host codex|claude-code --entity Atlas
--project-root .`를 실행하면 `migration_required`와 owned entry만
`--locked-entity`로 바꾸는 정확한 `brain-ai ... connect ... --apply` command가
나옵니다. Diff를 먼저 확인하려면 같은 connection command에서 `--apply`를 빼고
preview하세요.

Session 시작 시 자동 recall하고 변경이 있을 때만 checkpoint를 만들려면 project
connection command에 `--mode loop`를 추가하고 [autonomous loop
guide](08-autonomous-loop.ko.md)를 따르세요. 적용 대상인 global 또는 project-scoped
legacy rule이 quarantine 상태이면 host config를 하나도 바꾸기 전에 loop apply를
거부하며, `rule list`와 `doctor`가 해결 절차를 안내합니다.
...
## Host가 소유하는 tools-only integration pattern

기본 `--mode tools` connection에서는 연결하는 host가 다음 loop를 닫을 수 있지만
server가 이를 자동 schedule하거나 실행하지는 않습니다.
...
이 tools-only pattern은 host model의 conversation history를 대체하지 않습니다.
Framework session store가 chat transcript를 계속 소유하고 host가 Brain-AI의
differentiated operational memory에 기록할 내용을 선택해야 합니다. Action policy는
host가 enforcement boundary를 배선하기 전까지 advisory입니다. 별도의
project-only `--mode loop` integration은 bounded recall, 선택된 artifact event 기록,
dirty-only checkpoint를 자동화하지만 raw transcript를 저장하거나 model output에서
durable truth를 추론하지는 않습니다.

...
The public repository now includes a local-first reference implementation of
the component contracts. It is intentionally small, provider-neutral, and
dependency-free. It is usable code, but it is still an alpha reference rather
than a hosted multi-tenant service.
...
| `state.sqlite3` | semantic memory, rules, numerical state, lifecycle records |
...
```bash
brain-ai remember --type episodic --text "The release window moved to Thursday" --promote semantic
brain-ai remember --type semantic --text "Production releases require review"
brain-ai remember --type state --key open_reviews --value 3
brain-ai remember --type rule --pattern 'deploy\s+production' --text "approval required"
```
